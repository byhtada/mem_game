...
4. **RestartChannel** (`app/channels/restart_channel.rb`)
   - Обрабатывает подписки на обновления рестарта игры
   - Проверяет права доступа пользователя к игре
   - Автоматически отправляет текущее состояние рестарта при подписке

5. **Connection** (`app/channels/application_cable/connection.rb`)
   - Аутентификация через `user_id` параметр
   - Идентификация пользователей для Action Cable

6. **Game Model** (`app/models/game.rb`)
   - Метод `broadcast_game_update()` для отправки обновлений
   - Метод `broadcast_restart_update()` для отправки обновлений рестарта
   - Автоматическое broadcasting при изменениях состояния игры
   - Broadcasting при добавлении игроков

7. **Round Model** (`app/models/round.rb`)
...
8. **Маршруты** (`config/routes.rb`)
   - Добавлен endpoint `/cable` для WebSocket соединений

9. **RestartBroadcastJob** (`app/jobs/restart_broadcast_job.rb`)
   - Отдельная очередь `:restart_queue` для обработки рестарта
   - Автоматическое планирование повторных broadcasts
   - Умная логика остановки при завершении рестарта
...
2. **Обновленная логика игры** (`memgame_web/js/app.js`)
   ...
   - Функция `subscribeToRestartUpdates()` для WebSocket подписки на рестарт
   - Fallback на polling при проблемах с WebSocket
   - Общие обработчики `handleGameUpdate()`, `handleRoundUpdate()`, `handleVoteUpdate()` и `handleRestartUpdate()` для обоих методов
...
### Фаза рестарта:
1. При завершении игры вызывается `timeoutRestartWait()`
2. Если веб-сокеты включены, создается подписка на `RestartChannel`
3. Запускается `RestartBroadcastJob` в отдельной очереди `:restart_queue`
4. При готовности к рестарту в модели Game вызывается `broadcast_restart_update`
5. Все подписанные клиенты получают real-time обновления о прогрессе рестарта
6. При завершении рестарта подписка отключается и запускается новая игра

### Общие принципы:
7. При проблемах с WebSocket автоматически переключается на polling
